    ...
    for v in varx2:
        ...
        if v.isnumeric():
            ...
            for vv in database3.keys():
                vx=vv.upper()
                if v == vx:
                    listaNum_.append(database3[vv])
    x=""
    if len(lista_)==len(listaNum_) and len(lista_)!=0:
        for elem in range(len(lista_)):
            x+=lista_[elem]+" "+str(listaNum_[elem])+" "
